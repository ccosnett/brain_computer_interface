BeginPackage["corticalColumn1`"];
Unprotect["corticalColumn1`*"]; ClearAll["corticalColumn1`*"]; ClearAll["corticalColumn1`Private`*"];


reflex::usage = "reflex[name] contains the kernel definition for the shortcut name. For a list of shortcuts type ?Shortcuts";
...
Begin["`Private`"];
 ...
 abilityList = {"SelectCellBracket", "Minimize", "EvaluateNotebook",
  ...
  "TestRun1", "TestRun2", "F6", "RunJoker", "OpenJoker", 
  "EvaluateSelected", "OpenInit.m", "OpenInit.m", "OpenInit.m"};

reflex["SelectCellBracket"] :=
...
reflex["DeleteOutputAndMessages"] :=
...
reflex["DeleteAllCellsButInputAndCode"] :=
...
reflex["QuitFrontEnd", enb_:EvaluationNotebook[]] :=
...
reflex["RestartFrontEnd", enb_:EvaluationNotebook[]] :=
...
(* This is a slight rewrite of Kuba's answer here: http://mathematica.stackexchange.com/a/55073/29
   ...
   Therefore I prefer to user the version below using reflex["SelectToTop"]

reflex["EvaluateFromTop"] :=  Module[{i, enb = EvaluationNotebook[]},
...
Shortcuts`reflex["EvaluateFromTop"] :=  (
reflex["SelectToTop"];
...
SetAttributes[reflex, HoldRest];
reflex["SelectToTop", lastCellFunction_: (CellEvaluationFunction :> Identity)] :=
...
reflex["DoIt"] := (
	...
	If[("FileName" /. NotebookInformation[EvaluationNotebook[]]) =!= "FileName",
		reflex["SelectToTop", CellEvaluationFunction :> ( (
	...
	reflex["RestartKernel"]; (* restart the kernel *)
...
reflex["TestRun1"] := (
	...
	NotebookDelete @ Cells[ CellStyle -> ("Output"|"Print"|"Message")]; (* usually this makes sense *)
	NotebookDelete @ Cells @ MessagesNotebook[]; (* and this too *)
	reflex["SelectToTop"];   (* select all cells from the cursor up to the beginning *)
...
reflex["OpenInit.m"] :=  NotebookOpen@FindFile["init.m"];

reflex["OpenUserBaseDirectory"] :=  SystemOpen[$UserBaseDirectory];

reflex["OpenShortcutHelpPage"] := (
...
reflex["wrap"]:=NotebookApply[SelectedNotebook[], RowBox[{"[", "\[SelectionPlaceholder]", "]"}], Before];

reflex["texCopy"] := (FrontEnd`CopyToClipboard[FrontEnd`CopyAsTeX[]];
